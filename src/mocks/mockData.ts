...
// ─── Blockchain fields ────────────────────────────────────────────────────────
// These come from your backend after the market is created on-chain.
// conditionId  → unique ID of this market on the ConditionalTokens contract
// yesTokenId   → ERC-1155 token ID representing the YES outcome
// noTokenId    → ERC-1155 token ID representing the NO outcome
// collateralToken → the token used to trade (POL native = "native", or ERC-20 address)
//
// For dummy data we use realistic-looking IDs.
// Real IDs are 32-byte hex strings generated by the ConditionalTokens contract.
// Your backend dev will provide these when markets are created on-chain.

const COLLATERAL = "native" // using POL (native token) — change to USDC address if needed

// each market gets unique condition + token IDs
// in production these come from the ConditionalTokens contract deployment
...
export const RAW_MARKETS: Market[] = [
  ...
  },
  {
    ...
    orderBook: DUMMY_ORDER_BOOK,
    rules: DUMMY_RULES,
    conditionId: MARKET_CHAIN_DATA[1]?.yesTokenId,
